...
def test_runtime_and_documents_roots_reject_any_overlap(tmp_path: Path, state_root: str, documents_root: str) -> None:
    ...
    with pytest.raises(DocumentsPlanePathError, match="overlap"):
        ensure_runtime_state_root(tmp_path / state_root, documents_root=tmp_path / documents_root)
...
def test_runtime_and_documents_sibling_roots_are_allowed(tmp_path: Path) -> None:
    ...
    assert actual == state_root.resolve()
    assert (
        resolve_runtime_write_path(state_root, "evidence/receipt.json", documents_root=documents_root)
