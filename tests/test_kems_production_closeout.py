...
def _write_bundle(tmp_path: Path, *, run_id: str = "run-001") -> tuple[Path, Path, Path]:
...
def test_validate_closeout_binds_all_redacted_artifacts(tmp_path: Path) -> None:
    ...
    result = validate_closeout(preflight_path=paths[0], manifest_path=paths[1], receipt_path=paths[2])
...
def test_validate_closeout_rejects_mismatched_receipt(tmp_path: Path, field: str) -> None:
    paths = _write_bundle(tmp_path)
    ...
    with pytest.raises(CloseoutError):
        validate_closeout(preflight_path=paths[0], manifest_path=paths[1], receipt_path=paths[2])
