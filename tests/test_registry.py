...
class TestAgentInfo:
    ...
    def test_can_handle(self):
        agent = AgentInfo(
            capabilities=[
                Capability(name="code-generation", tags=["python"]),
                Capability(name="deploy"),
            ]
        )
...
class TestRegistryStore:
    ...
    def test_find_by_capability(self):
        ...
        store.register_agent(
            AgentInfo(name="coder", capabilities=[Capability(name="code")])
        )
        store.register_agent(
            AgentInfo(name="deployer", capabilities=[Capability(name="deploy")])
        )
    ...
    def test_heartbeat(self):
        ...
        assert store.get_agent(agent.agent_id).last_heartbeat > old_hb  # type: ignore[reportOptionalMemberAccess]
...
class TestHeartbeatManager:
    def test_marks_stale_offline(self):
        ...
        assert store.get_agent(agent.agent_id).status == AgentStatus.OFFLINE  # type: ignore[reportOptionalMemberAccess]
...
class TestRegistryServer:
    ...
    def test_sync_delta_merges_remote_agent(self, client):
        remote = AgentInfo(
            name="remote", node_id="node-b", capabilities=[Capability(name="code")]
        )
        r = client.post(
            "/sync/delta",
            json={
                ...
                "vclock": 4,
                "agents": [remote.to_dict()],
            },
        )
        assert r.status_code == 200
        assert r.json()["merged"] == 1
        assert (
            client.get("/agents/find", params={"node_id": "node-b"}).json()[0]["name"]
            == "remote"
        )
...
class TestDispatcher:
    def test_dispatch_to_capable_agent(self):
        ...
        assignment = dispatcher.submit(
            TaskRequest(name="t1", required_capabilities=["code"])
        )
    ...
    def test_dispatch_no_capable_agent(self):
        ...
        dispatcher = Dispatcher(store)
        assignment = dispatcher.submit(
            TaskRequest(name="t1", required_capabilities=["deploy"])
        )
    ...
    def test_dispatch_respects_concurrency(self):
        ...
        store.register_agent(
            AgentInfo(
                name="coder", capabilities=[Capability(name="code")], max_concurrency=1
            )
        )
        dispatcher = Dispatcher(store)
        dispatcher.submit(TaskRequest(name="t1", required_capabilities=["code"]))
        assignment = dispatcher.submit(
            TaskRequest(name="t2", required_capabilities=["code"])
        )
        assert assignment is None
    ...
    def test_complete_frees_agent(self):
        ...
        assert a.active_tasks == 0  # type: ignore[reportOptionalMemberAccess]
        assert a.status == AgentStatus.IDLE  # type: ignore[reportOptionalMemberAccess]
    ...
    def test_stats(self):
        store = RegistryStore()
        store.register_agent(
            AgentInfo(name="c", capabilities=[Capability(name="code")])
        )
        dispatcher = Dispatcher(store)
        dispatcher.submit(TaskRequest(name="t1", required_capabilities=["code"]))
        s = dispatcher.stats()
